	...
	<table id="page-length-option" class="display table-browse">
		...
		<tbody>
			...
			<tr>
				...
				<td>{{$canal->name}}</td>
				<td>
					<a href="{{route('canal.edit', $canal->slug())}}" class="btn-floating waves-effect waves-light left mr-2 green darken-1 tooltipped" data-position="top" data-tooltip="{{Lang::get('common.controller_edit')}}">
						<i class="material-icons">edit</i>
					</a>

					{{--
					<form id="delete-company" action="{{ route('canal.destroy', $canal->slug()) }}" method="post">
						{{csrf_field()}}
						@method('DELETE')

	...
	<table id="page-length-option" class="display table-browse">
		...
		<tbody>
			...
			<tr>
				...
				<td>{{$origin->name}}</td>
				<td>
					<a href="{{route('origin.edit', $origin->slug())}}" class="btn-floating waves-effect waves-light left mr-2 green darken-1 tooltipped" data-position="top" data-tooltip="{{Lang::get('common.controller_edit')}}">
						<i class="material-icons">edit</i>
					</a>
					<form id="delete-company" action="{{ route('origin.destroy', $origin->slug()) }}" method="post">
						{{csrf_field()}}
						@method('DELETE')

...
<div class="card-content">
	<table id="page-length-option" class="display table-browse">
		...
		<tbody>
			@forelse($naturezaEstatisticas as $natureza)
			<tr>
				<td>{{$natureza->id}}</td>
				<td>{{$natureza->name}}</td>
				<td>
					<a href="{{route('natureza-estatistica.edit', $natureza->slug())}}" class="btn-floating waves-effect waves-light left mr-2 green darken-1 tooltipped" data-position="top" data-tooltip="{{Lang::get('common.controller_edit')}}">
						<i class="material-icons">edit</i>
					</a>
					<form id="delete-company" action="{{ route('natureza-estatistica.destroy', $natureza->slug()) }}" method="post">
						{{csrf_field()}}
						@method('DELETE')

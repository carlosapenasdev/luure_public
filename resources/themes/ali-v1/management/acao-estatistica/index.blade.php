...
<div class="card-content">
	<table id="page-length-option" class="display table-browse">
		...
		<tbody>
			@forelse($acaoEstatisticas as $acao)
			<tr>
				<td>{{$acao->id}}</td>
				<td>{{$acao->name}}</td>
				<td>
					<a href="{{route('acao-estatistica.edit', $acao->slug())}}" class="btn-floating waves-effect waves-light left mr-2 green darken-1 tooltipped" data-position="top" data-tooltip="{{Lang::get('common.controller_edit')}}">
						<i class="material-icons">edit</i>
					</a>
					<form id="delete-company" action="{{ route('acao-estatistica.destroy', $acao->slug()) }}" method="post">
						{{csrf_field()}}
						@method('DELETE')

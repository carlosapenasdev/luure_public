...
<div class="card-content">
	<table id="page-length-option" class="display table-browse">
		...
		<tbody>
			@forelse($origemEstatisticas as $origem)
			<tr>
				<td>{{$origem->id}}</td>
				<td>{{$origem->name}}</td>
				<td>
					<a href="{{route('origem-estatistica.edit', $origem->slug())}}" class="btn-floating waves-effect waves-light left mr-2 green darken-1 tooltipped" data-position="top" data-tooltip="{{Lang::get('common.controller_edit')}}">
						<i class="material-icons">edit</i>
					</a>
					<form id="delete-company" action="{{ route('origem-estatistica.destroy', $origem->slug()) }}" method="post">
						{{csrf_field()}}
						@method('DELETE')
